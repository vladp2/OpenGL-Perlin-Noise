...
bool ReadModel(std::string modelFile) {
    modelFile = "Game/models/" + modelFile + ".vertex";

	std::ifstream vertexStream(modelFile, std::ios::in);
	if (!vertexStream.is_open()) {
        std::cout << "Impossible to open " << modelFile << '\n';
		return false;
	}
    ...
    model[modelCount].data = new float[vec.size()];
    copy(vec.begin(), vec.end(), model[modelCount].data);
    model[modelCount].size = vec.size();
...
bool ReadTexture(std::string textureFile) {
    textureFile = "Game/textures/" + textureFile + ".bmp";
